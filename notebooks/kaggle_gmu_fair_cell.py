# ========== BrainDrainDetector — Kaggle ONE CELL: 2-mod GMU fair ==========
#
# 2-modality Gated Multimodal Unit (audio vs combined bio embedding).
# Fair comparison vs cross_attn_pooled_weighted_no_aug (F1 ~0.185).
#
#   fusion_mode: gated_multimodal_unit
#   dual_tower_biosignal: false   (single BiGRU on all 6 channels — like baseline)
#   weighted_loss + balanced_sampling + macro_f1 + batch 8 + no aug
#   gate_reg_lambda: 0.01, audio_bias_init: 0.67
#
...
#   src/05_train.py
#   configs/base.yaml
#   configs/exp_gmu_weighted_no_aug.yaml
...
#
# ── Output zip ─────────────────────────────────────────────────────────────
#   results_classification_gmu_weighted_no_aug.zip
...
CELL_VERSION = "2026-06-16-gmu-fair-onecell-v1"
ZIP_OUTPUT_NAME = "results_classification_gmu_weighted_no_aug.zip"
EXP_CONFIG_NAME = "exp_gmu_weighted_no_aug.yaml"
...
INLINE_EXP_OVERRIDES = {
    "model": {
        ...
        "gmu": {
            "gate_reg_enabled": True,
            "gate_reg_lambda": 0.01,
            "audio_bias_init": 0.67,
            "use_quality_inputs": False,
        },
...
print(f"BrainDrainDetector — 2-mod GMU fair | {CELL_VERSION}")
...
def verify_repo() -> None:
    ...
    if "GatedMultimodalFusion" not in fusion_src:
        raise RuntimeError("fusion.py lacks GatedMultimodalFusion — push latest src/.")
    train_src = (REPO / "src/05_train.py").read_text(encoding="utf-8")
    if "_gmu_gate_regularization" not in train_src:
        raise RuntimeError("05_train.py lacks GMU gate reg — push latest src/.")
...
def load_cfg(kemocon_root: Path) -> OmegaConf:
    ...
    if exp_path.is_file():
        ...
        print(f"[Kaggle] Using inline experiment overrides.")
...
def zip_results(cfg, run_config: Path) -> Path:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        manifest = {
            ...
            "experiment": "gmu_weighted_no_aug",
            "fusion_mode": "gated_multimodal_unit",
            "dual_tower_biosignal": False,
            "gmu_gate_reg_lambda": 0.01,
            "gmu_audio_bias_init": 0.67,
            "compare_to_baseline": "results_classification_cross_attn_pooled_weighted_no_aug",
        }
...
print(f"Dataset: debates={n_debates}")
...
processed = Path(cfg.paths.data_processed)
cfg_path = Path("/kaggle/working/kaggle_gmu_fair_config.yaml")
run_config = Path("/kaggle/working/kaggle_run_config.yaml")
OmegaConf.save(cfg, cfg_path)
OmegaConf.save(cfg, run_config)

print("\n[Fair protocol — 2-mod GMU]")
print(f"  fusion_mode: {cfg.model.fusion_mode}")
print(f"  dual_tower_biosignal: {cfg.model.dual_tower_biosignal}")
print(f"  gmu gate_reg: {cfg.model.gmu.gate_reg_enabled} (lambda={cfg.model.gmu.gate_reg_lambda})")
print(f"  gmu audio_bias_init: {cfg.model.gmu.audio_bias_init}")
