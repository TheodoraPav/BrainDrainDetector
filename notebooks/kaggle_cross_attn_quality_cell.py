# ========== BrainDrainDetector — Kaggle ONE CELL: quality-aware cross-attention ==========
#
# Fair comparison vs results_classification_cross_attn_pooled_weighted_no_aug (F1 ~0.185)
# ONLY changes vs baseline:
#   cross_attn.quality_aware: true
#   cross_attn.quality_supervision.lambda: 0.1
...
#   src/data/dataset.py
#   configs/base.yaml
#   configs/exp_cross_attn_quality_weighted_no_aug.yaml
...
#
# ── Output zip ─────────────────────────────────────────────────────────────
#   results_classification_cross_attn_quality_weighted_no_aug.zip
...
CELL_VERSION = "2026-06-16-cross-attn-quality-onecell-v1"
ZIP_OUTPUT_NAME = "results_classification_cross_attn_quality_weighted_no_aug.zip"
EXP_CONFIG_NAME = "exp_cross_attn_quality_weighted_no_aug.yaml"
...
print(f"BrainDrainDetector — quality-aware cross-attention | {CELL_VERSION}")
...
def verify_repo() -> None:
    ...
    if "quality_aware" not in fusion_src or "trust_gate" not in fusion_src:
        raise RuntimeError("fusion.py lacks quality-aware cross-attn — push latest src/.")
    train_src = (REPO / "src/05_train.py").read_text(encoding="utf-8")
    if "_cross_attn_quality_supervision_loss" not in train_src:
        raise RuntimeError("05_train.py lacks quality supervision loss — push latest src/.")
    if "_add_fusion_auxiliary_losses" not in train_src:
        raise RuntimeError("05_train.py lacks AMP-safe auxiliary losses — push latest src/.")
...
def zip_results(cfg, run_config: Path) -> Path:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        manifest = {
            ...
            "experiment": "cross_attn_quality_weighted_no_aug",
            "fusion_mode": "cross_attn_pooled",
            "cross_attn_quality_aware": True,
            "quality_supervision_lambda": 0.1,
...
def print_loso_summary(processed: Path) -> None:
    ...
    print("\n=== LOSO summary (quality-aware cross-attn) ===")
...
processed = Path(cfg.paths.data_processed)
cfg_path = Path("/kaggle/working/kaggle_cross_attn_quality_config.yaml")
run_config = Path("/kaggle/working/kaggle_run_config.yaml")
OmegaConf.save(cfg, cfg_path)
OmegaConf.save(cfg, run_config)

print("\n[Fair protocol — quality-aware cross-attn]")
print(f"  fusion_mode: {cfg.model.fusion_mode}")
print(f"  quality_aware: {cfg.model.cross_attn.quality_aware}")
print(f"  quality_supervision lambda: {cfg.model.cross_attn.quality_supervision['lambda']}")
