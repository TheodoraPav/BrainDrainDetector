# ========== BrainDrainDetector — Kaggle CELL 2: CLASSIFICATION ABLATIONS ==========
#
# Run AFTER Cell 1 (kaggle_baseline_one_cell.py) or with seeded audio+physio on dataset.
# Full experiment matrix → notebooks/KAGGLE_EXPERIMENT_PLAN.md
#
# Set PRESET to a missing run name, OR leave None and edit knobs manually (one change per run).
# Fair defaults: macro_f1 | batch 8 | patience 8 | weighted CE | no aug
#
...
# =============================================================================
# Presets:
#   offline_aug         pooled + weighted CE + balanced sampling + offline aug (step 04)
#   gmu_fair            GMU fusion + weighted + macro_f1 (vs cross_attn_pooled)
#   cross_attn_balanced_fair  balanced residuals + modality dropout (fix audio bias)
#   pooled_unweighted   pooled baseline, NO weighted loss / NO balanced sampling
#   pooled_lstm5_fair   pooled + LSTM×5 + macro_f1
#   seq_cnn_fair / seq_lstm5_fair — legacy sequence branch
# None = manual knobs below
PRESET: str | None = "offline_aug"

CELL_VERSION = "2026-06-11-v2-classification-cell2"
...
USE_AMP = True
RUN_EXPLAIN = False                 # True → slow 07_explain.py (attention maps)

CROSS_ATTN_BALANCED_RESIDUAL = False
MODALITY_DROPOUT_ENABLED = False
MODALITY_DROPOUT_P = 0.15

SAVE_RESULTS_ZIP = True
ZIP_OUTPUT_NAME: str | None = None  # None → auto name from fusion/cnn/temporal/weighted/aug
...
AUTO_FORCE_RERUN = True             # when FORCE_RERUN_STEPS empty → preset or ["05", "06"]
# =============================================================================

TASK_MODE = "classification"  # fixed — this cell is classification-only

_PRESET_KNOBS: dict[str, dict] = {
    "pooled_unweighted": {
        ...
        "CELL_VERSION": "2026-06-11-pooled-unweighted-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_cross_attn_pooled_unweighted_no_aug.zip",
    },
    "seq_cnn_fair": {
        ...
        "AUGMENTATION_ENABLED": False,
        "FORCE_RERUN_STEPS": ["05", "06"],
        "CELL_VERSION": "2026-06-11-seq-cnn-fair-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_sequence_cross_attn_cnn_weighted_no_aug_final.zip",
    },
    "seq_lstm5_fair": {
        "FUSION_MODE": "sequence_cross_attn",
        "PHYSIO_CNN_ENABLED": False,
        ...
        "AUGMENTATION_ENABLED": False,
        "FORCE_RERUN_STEPS": ["05", "06"],
        "CELL_VERSION": "2026-06-11-seq-lstm5-fair-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_sequence_cross_attn_lstm5_weighted_no_aug_final.zip",
    },
    "pooled_lstm5_fair": {
        "FUSION_MODE": "cross_attn_pooled",
        "PHYSIO_CNN_ENABLED": False,
        ...
        "AUGMENTATION_ENABLED": False,
        "FORCE_RERUN_STEPS": ["05", "06"],
        "CELL_VERSION": "2026-06-11-pooled-lstm5-fair-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_cross_attn_pooled_lstm5_weighted_no_aug_final.zip",
    },
    "offline_aug": {
        "FUSION_MODE": "cross_attn_pooled",
        "PHYSIO_CNN_ENABLED": False,
        ...
        "WEIGHTED_LOSS": True,
        "BALANCED_SAMPLING": True,
        "FORCE_RERUN_STEPS": ["04", "05", "06"],
        "CELL_VERSION": "2026-06-11-pooled-weighted-balanced-aug-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_cross_attn_pooled_weighted_aug.zip",
    },
    "gmu_fair": {
        "FUSION_MODE": "gated_multimodal_unit",
        "PHYSIO_CNN_ENABLED": False,
        "DUAL_TOWER_BIOSIGNAL": False,
        "TEMPORAL_MODE": None,
        "AUGMENTATION_ENABLED": False,
        ...
        "BALANCED_SAMPLING": True,
        "FORCE_RERUN_STEPS": ["05", "06"],
        "CELL_VERSION": "2026-06-15-gmu-weighted-macro-f1",
        "ZIP_OUTPUT_NAME": "results_classification_gmu_weighted_no_aug.zip",
        "RUN_EXPLAIN": True,
    },
    "cross_attn_balanced_fair": {
        "FUSION_MODE": "cross_attn_pooled",
        "PHYSIO_CNN_ENABLED": False,
        "DUAL_TOWER_BIOSIGNAL": False,
        "TEMPORAL_MODE": None,
        ...
        "BALANCED_SAMPLING": True,
        "FORCE_RERUN_STEPS": ["05", "06"],
        "CELL_VERSION": "2026-06-15-cross-attn-balanced-macro-f1",
...
if PRESET is not None:
    if PRESET not in _PRESET_KNOBS:
        raise ValueError(f"Unknown PRESET={PRESET!r}. Choose from {list(_PRESET_KNOBS)}")
    _pk = _PRESET_KNOBS[PRESET]
    FUSION_MODE = _pk["FUSION_MODE"]
    PHYSIO_CNN_ENABLED = _pk["PHYSIO_CNN_ENABLED"]
    DUAL_TOWER_BIOSIGNAL = _pk["DUAL_TOWER_BIOSIGNAL"]
    TEMPORAL_MODE = _pk["TEMPORAL_MODE"]
    AUGMENTATION_ENABLED = _pk["AUGMENTATION_ENABLED"]
    FORCE_RERUN_STEPS = list(_pk["FORCE_RERUN_STEPS"])
    if "WEIGHTED_LOSS" in _pk:
        WEIGHTED_LOSS = bool(_pk["WEIGHTED_LOSS"])
    if "BALANCED_SAMPLING" in _pk:
        BALANCED_SAMPLING = bool(_pk["BALANCED_SAMPLING"])
    CELL_VERSION = _pk.get("CELL_VERSION", CELL_VERSION)
    if _pk.get("ZIP_OUTPUT_NAME"):
        ZIP_OUTPUT_NAME = _pk["ZIP_OUTPUT_NAME"]
    if "RUN_EXPLAIN" in _pk:
        RUN_EXPLAIN = bool(_pk["RUN_EXPLAIN"])
    if "CROSS_ATTN_BALANCED_RESIDUAL" in _pk:
        CROSS_ATTN_BALANCED_RESIDUAL = bool(_pk["CROSS_ATTN_BALANCED_RESIDUAL"])
    if "MODALITY_DROPOUT_ENABLED" in _pk:
        MODALITY_DROPOUT_ENABLED = bool(_pk["MODALITY_DROPOUT_ENABLED"])
    if "MODALITY_DROPOUT_P" in _pk:
        MODALITY_DROPOUT_P = float(_pk["MODALITY_DROPOUT_P"])
    print(f"[Kaggle] PRESET applied: {PRESET}")

if AUTO_FORCE_RERUN and not FORCE_RERUN_STEPS:
    FORCE_RERUN_STEPS = ["05", "06"]

print(f"[Kaggle] classification baseline | fusion={FUSION_MODE} | weighted={WEIGHTED_LOSS} | balanced_sampling={BALANCED_SAMPLING} | FORCE_RERUN_STEPS={FORCE_RERUN_STEPS}")
...
if FUSION_MODE == "gated_multimodal_unit":
    if not hasattr(cfg.model, "gmu") or cfg.model.gmu is None:
        cfg.model.gmu = OmegaConf.create({})
    cfg.model.gmu.gate_reg_enabled = True
    cfg.model.gmu.gate_reg_lambda = 0.01
    cfg.model.gmu.audio_bias_init = 0.67
    cfg.model.gmu.use_quality_inputs = False
if not hasattr(cfg.model, "cross_attn") or cfg.model.cross_attn is None:
    cfg.model.cross_attn = OmegaConf.create({})
cfg.model.cross_attn.balanced_residual = bool(CROSS_ATTN_BALANCED_RESIDUAL)
if not hasattr(cfg.model, "modality_dropout") or cfg.model.modality_dropout is None:
    cfg.model.modality_dropout = OmegaConf.create({})
cfg.model.modality_dropout.enabled = bool(MODALITY_DROPOUT_ENABLED)
cfg.model.modality_dropout.p = float(MODALITY_DROPOUT_P)
...
print("  early_stopping_patience:", cfg.training.early_stopping_patience,
      "| min_epochs:", cfg.training.early_stopping_min_epochs)
...
_print_loso_summary(PROCESSED / "loso_results.pt", f"LOSO summary (classification, {FUSION_MODE})")

print("\nOutputs:")
print("  data_processed:", cfg.paths.data_processed)
print("  checkpoints:   ", cfg.paths.checkpoints)
print("  figures:       ", cfg.paths.figures)
print("\n[Kaggle] CLASSIFICATION CELL 2 DONE.")
_SAVE_HINTS = {
    "offline_aug": "results/results_classification_cross_attn_pooled_weighted_aug/",
    "pooled_unweighted": "results/results_classification_cross_attn_pooled_unweighted_no_aug/",
    "seq_cnn_fair": "results/results_classification_sequence_cross_attn_cnn_weighted_no_aug_final/",
    "seq_lstm5_fair": "results/results_classification_sequence_cross_attn_lstm5_weighted_no_aug_final/",
    "pooled_lstm5_fair": "results/results_classification_cross_attn_pooled_lstm5_weighted_no_aug_final/",
}
if PRESET in _SAVE_HINTS:
    print("Save zip to", _SAVE_HINTS[PRESET])
