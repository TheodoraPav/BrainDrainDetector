# ========== BrainDrainDetector — Kaggle ONE CELL: cross-attn + physio feature MLP ==========
#
# Fair comparison vs results_classification_cross_attn_pooled_weighted_no_aug (best single model)
# ONLY change vs baseline:
#   physio_encoder: feature_mlp  (15 hand-crafted features + MLP → 256-d bio embedding)
# Same: cross_attn_pooled, weighted_loss, balanced_sampling, macro_f1, batch 8, no aug
#
# ── BEFORE RUN: git push master with these files ─────────────────────────────
#   src/utils/physio_features.py
#   src/models/biosignal_encoder.py
#   src/models/classifier.py
#   src/03_preprocess_physio.py
#   src/04_build_tensors.py
#   src/data/dataset.py
#   src/05_train.py
#   configs/base.yaml
#   configs/exp_cross_attn_feature_mlp_weighted_no_aug.yaml
...
#
# ── Output zip ─────────────────────────────────────────────────────────────
#   results_classification_cross_attn_feature_mlp_weighted_no_aug.zip
...
CELL_VERSION = "2026-06-18-cross-attn-feature-mlp-onecell-v1"
ZIP_OUTPUT_NAME = "results_classification_cross_attn_feature_mlp_weighted_no_aug.zip"
EXP_CONFIG_NAME = "exp_cross_attn_feature_mlp_weighted_no_aug.yaml"
...
INLINE_EXP_OVERRIDES = {
    "model": {
        ...
        "physio_encoder": "feature_mlp",
        "physio_feature_dim": 15,
        "feature_mlp_dropout": 0.1,
        "cross_attn": {
...
# Must rebuild physio (features) + windows + train
FORCE_RERUN_STEPS = ["03", "04", "05", "06"]
SKIP_IF_EXISTS = True

print(f"BrainDrainDetector — cross-attn + physio feature MLP | {CELL_VERSION}")
...
def verify_repo() -> None:
    required = [
        REPO / "src/utils/physio_features.py",
        REPO / "src/models/biosignal_encoder.py",
        REPO / "src/models/classifier.py",
        REPO / "src/03_preprocess_physio.py",
        REPO / "src/04_build_tensors.py",
    ...
    feat_src = (REPO / "src/utils/physio_features.py").read_text(encoding="utf-8")
    if "PHYSIO_FEATURE_DIM" not in feat_src or "extract_physio_features" not in feat_src:
        raise RuntimeError("physio_features.py missing — push latest src/.")
    enc_src = (REPO / "src/models/biosignal_encoder.py").read_text(encoding="utf-8")
    if "PhysioFeatureMLPEncoder" not in enc_src:
        raise RuntimeError("biosignal_encoder.py lacks PhysioFeatureMLPEncoder — push latest src/.")
    cls_src = (REPO / "src/models/classifier.py").read_text(encoding="utf-8")
    if "physio_encoder" not in cls_src or "physio_features" not in cls_src:
        raise RuntimeError("classifier.py lacks feature_mlp wiring — push latest src/.")
    exp_path = REPO / "configs" / EXP_CONFIG_NAME
    if exp_path.is_file():
        print(f"[Kaggle] Repo check OK (found {EXP_CONFIG_NAME}).")
    else:
        print(f"[Kaggle] Repo check OK — {EXP_CONFIG_NAME} missing; using INLINE_EXP_OVERRIDES.")
...
def load_cfg(kemocon_root: Path) -> OmegaConf:
    ...
    if exp_path.is_file():
        ...
        print("[Kaggle] Using inline experiment overrides.")
...
def step_done(step: str, cfg, processed: Path, n_debates: int) -> bool:
    ...
    if step == "03":
        physio = processed / "physio"
        if _count(physio, "*.pt") == 0:
            return False
        sample = next(physio.glob("*.pt"), None)
        if sample is None:
            return False
        import torch
        data = torch.load(sample, weights_only=True)
        return "physio_features" in data
    if step == "04":
        wd = processed / ("windows_aug" if cfg.augmentation.enabled else "windows")
        if _count(wd, "*.pt") == 0:
            return False
        sample = next(wd.glob("*.pt"), None)
        if sample is None:
            return False
        import torch
        data = torch.load(sample, weights_only=True)
...
def force_clean(cfg) -> None:
    ...
    if "03" in FORCE_RERUN_STEPS:
        physio = processed / "physio"
        if physio.is_dir():
            shutil.rmtree(physio)
            print("  cleared physio/ (force rebuild with physio_features)")
    if "04" in FORCE_RERUN_STEPS:
        for name in ("windows", "windows_aug"):
            wd = processed / name
            if wd.is_dir():
                shutil.rmtree(wd)
...
def zip_results(cfg, run_config: Path) -> Path:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        manifest = {
            ...
            "experiment": "cross_attn_feature_mlp_weighted_no_aug",
            "fusion_mode": "cross_attn_pooled",
            "physio_encoder": "feature_mlp",
            "physio_feature_dim": 15,
...
def print_loso_summary(processed: Path) -> None:
    ...
    print("\n=== LOSO summary (cross-attn + feature MLP) ===")
    for k, v in data.get("summary", {}).items():
        if isinstance(v, float):
            print(f"  {k}: {v:.4f}")
        else:
            print(f"  {k}: {v}")
    print("\nCompare to baseline cross_attn_pooled_weighted_no_aug (macro F1 ~0.547, alarm TP ~51%)")
...
processed = Path(cfg.paths.data_processed)
cfg_path = Path("/kaggle/working/kaggle_cross_attn_feature_mlp_config.yaml")
run_config = Path("/kaggle/working/kaggle_run_config.yaml")
OmegaConf.save(cfg, cfg_path)
OmegaConf.save(cfg, run_config)

print("\n[Fair protocol — cross-attn + physio feature MLP]")
print(f"  fusion_mode: {cfg.model.fusion_mode}")
print(f"  physio_encoder: {cfg.model.get('physio_encoder', 'bigru')}")
print(f"  physio_feature_dim: {cfg.model.get('physio_feature_dim', 15)}")
...
PIPELINE = [
    ...
    ("03", "03_preprocess_physio.py", "physio + features"),
