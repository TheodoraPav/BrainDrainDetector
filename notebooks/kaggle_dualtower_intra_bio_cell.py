# ========== BrainDrainDetector — Kaggle ONE CELL: dual-tower intra-bio cross-attn ==========
#
# Hierarchical fusion (fair protocol):
#   1) E4 tower -> e4_emb  --\
#                             +-> intra-bio cross-attn (E4 queries EEG, 2 heads)
#   2) EEG tower -> eeg_emb -/
#   3) audio <-> fused_bio cross-attn (legacy / standard residual)
#
# Compare to:
#   baseline single-tower: F1 alarm ~0.185
#   legacy dual-tower concat: F1 alarm ~0.126
#
...
#   src/05_train.py
#   configs/base.yaml
#   configs/exp_cross_attn_dualtower_intra_bio_weighted_no_aug.yaml
...
#
# ── Output zip ─────────────────────────────────────────────────────────────
#   results_classification_cross_attn_dualtower_intra_bio_weighted_no_aug.zip
...
print(f"BrainDrainDetector — dual-tower intra-bio cross-attn | {CELL_VERSION}")
...
def zip_results(cfg, run_config: Path) -> Path:
    ...
    with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
        ...
        manifest = {
            ...
            "experiment": "cross_attn_dualtower_intra_bio_weighted_no_aug",
            "fusion_mode": "cross_attn_pooled",
            "dual_tower_biosignal": True,
            "dual_tower_intra_bio_fusion": "cross_attn",
            "dual_tower_intra_bio_num_heads": 2,
            "cross_attn_balanced_residual": False,
            "compare_to_baseline": "results_classification_cross_attn_pooled_weighted_no_aug",
            "compare_to_legacy_dualtower": "results_classification_cross_attn_pooled_dualtower_weighted_no_aug",
        }
...
def print_loso_summary(processed: Path) -> None:
    ...
    print("\n=== LOSO summary (dual-tower intra-bio) ===")
    for k, v in data.get("summary", {}).items():
        if isinstance(v, float):
            print(f"  {k}: {v:.4f}")
        else:
            print(f"  {k}: {v}")
    print("\nCompare: baseline F1 ~0.185 | legacy dual-tower concat F1 ~0.126")
...
processed = Path(cfg.paths.data_processed)
cfg_path = Path("/kaggle/working/kaggle_dualtower_intra_bio_config.yaml")
...
print(f"  intra_bio_fusion: {cfg.model.dual_tower.intra_bio_fusion}")
print(f"  intra_bio_num_heads: {cfg.model.dual_tower.intra_bio_num_heads}")
print(f"  audio-bio balanced_residual: {cfg.model.cross_attn.balanced_residual}")
