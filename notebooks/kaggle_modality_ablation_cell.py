# ========== BrainDrainDetector — Kaggle: AUDIO-ONLY + BIO-ONLY (2 zips) ==========
#
# One notebook run → sequential LOSO training for audio_only and bio_only,
# then two download zips for modality comparison vs full multimodal baseline.
#
# Requires pushed master with model.input_modality support (classifier.py).
#
# Fair settings (match pooled baseline): cross_attn_pooled | macro_f1 | batch 8 |
# patience 8 | weighted CE | balanced sampling | no aug
#
# ── Kaggle notebook setup ───────────────────────────────────────────────────
#   1. GPU T4 x1, Internet ON, HF_TOKEN secret, BrainDrainDataset attached
#   2. Push GitHub master BEFORE Run
#   3. Paste this entire file into ONE code cell and Run (~4–6 h total)
#
# Outputs:
#   results_classification_cross_attn_pooled_audio_only_weighted_no_aug.zip
...
# =============================================================================
# USER SETTINGS — fixed for dual modality ablation (edit only if you know why)
# =============================================================================
MODALITY_RUNS: list[str] = ["audio_only", "bio_only"]

MODALITY_ZIP_NAMES: dict[str, str] = {
    "audio_only": "results_classification_cross_attn_pooled_audio_only_weighted_no_aug.zip",
    "bio_only": "results_classification_cross_attn_pooled_bio_only_weighted_no_aug.zip",
}

for _mod in MODALITY_RUNS:
    if _mod not in MODALITY_ZIP_NAMES:
        raise ValueError(f"Missing zip name for modality {_mod!r} in MODALITY_ZIP_NAMES")

CELL_VERSION = "2026-06-11-modality-dual-audio-bio-macro-f1"
print(f"BrainDrainDetector Kaggle modality cell {CELL_VERSION}")
print(f"[Kaggle] Will train and zip: {MODALITY_RUNS}")
...
def apply_run_config(input_modality: str) -> None:
    ...
    global cfg, ACTIVE_INPUT_MODALITY

    if input_modality not in MODALITY_RUNS and input_modality != "full":
        raise ValueError(f"input_modality must be 'full' or one of {MODALITY_RUNS}; got {input_modality!r}")

    ACTIVE_INPUT_MODALITY = input_modality
    cfg = load_experiment_cfg()
...
def _prepare_modality_switch() -> None:
    """Clear train/eval artifacts before each audio_only / bio_only LOSO run."""
    global FORCE_RERUN_STEPS
    FORCE_RERUN_STEPS = ["05", "06"]
    _prepare_force_rerun_artifacts()


# Do not clear artifacts before preprocess — only between modality train runs.
...
# -------------------------
# 6) Pipeline — preprocess once, then train per modality
# -------------------------
PREPROCESS_PIPELINE = [
...
def _run_pipeline_steps(steps: list[tuple[str, str, str]], *, label: str) -> None:
    total = len(steps)
    for i, (step_id, script, description) in enumerate(steps, start=1):
        print("\n" + "=" * 80)
        print(f"{label} — STEP {i}/{total} ({step_id}) — {description}")
        print("=" * 80)
        if step_id == "05":
            import torch
...
print("\n" + "#" * 80)
print("# PREPROCESS (shared) — steps 01–04 once")
print("#" * 80)
_run_pipeline_steps(PREPROCESS_PIPELINE, label="PREPROCESS")

CREATED_ZIPS: list[str] = []

for modality in MODALITY_RUNS:
    print("\n" + "#" * 80)
    print(f"# MODALITY RUN: {modality}")
    print("#" * 80)
    apply_run_config(modality)
    _prepare_modality_switch()
    _run_pipeline_steps(TRAIN_PIPELINE, label=modality.upper())

    _print_loso_summary(
        PROCESSED / "loso_results.pt",
        f"LOSO summary ({modality}, {FUSION_MODE})",
    )

    if SAVE_RESULTS_ZIP:
        zip_name = MODALITY_ZIP_NAMES[modality]
        ...
        manifest_path = WORKING_ROOT / f"run_manifest_{modality}.json"
        ...
        zip_run_results(
            ...
            extra_files=[
                ...
                (manifest_path, f"run_manifest_{modality}.json"),
                (cfg_path, f"configs/{cfg_path.name}"),
            ],
            required_in_zip=["data_processed/loso_results.pt"],
        )
        CREATED_ZIPS.append(zip_name)
        print(f"Save locally to results/results_classification_cross_attn_pooled_{modality}_weighted_no_aug/")

if RUN_EXPLAIN:
    print("\n" + "=" * 80)
    print("OPTIONAL — attention explainability (skipped for single-modality runs)")
    print("=" * 80)
    run_step("07_explain.py", "07", extra=None)

print("\n[Kaggle] MODALITY ABLATION DONE.")
print("  Created zips:", CREATED_ZIPS)
print("  Compare macro_f1 vs full multimodal baseline (0.185).")
print("Download both zips from Kaggle Output tab (right sidebar).")
